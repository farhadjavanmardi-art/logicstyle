...
let _basisMode=false;  // Admin: neutrales Basis-Bild (Mannequin-Template) statt Kunden-Simulation erzeugen
...
async function doLogin(){
  ...
  try{
    ...
    updateCredits();switchGender('female');startHeartbeat();loadGalleryCounts();loadDeletedModels();
...
async function openCreditOffer(){
  ...
  document.getElementById('coBox').innerHTML=`
    ...
    ${next?`<div class="co-upgrade">💎 <b>Empfehlung: Upgrade auf ${next.label}</b><br>
      ...
      <button class="co-up-btn" onclick="sendCreditRequest('upgrade_request',{requested_plan:'${plan.next}'})">📈 Upgrade auf ${next.label} anfragen</button>
    ...
    ${packs.map(p=>`<div class="co-pack">
      ...
      <button class="co-pack-btn" onclick="sendCreditRequest('credit_purchase_request',{pack_amount:${Number(p.amount)},pack_price:${Number(p.price)}})">€${Number(p.price)} kaufen</button>
    </div>`).join('')}
    <div class="co-note">Nach der Anfrage melden wir uns umgehend zur Bezahlung — die Credits werden direkt danach freigeschaltet.</div>
...
async function sendCreditRequest(type,extra){
  ...
  const label=isUp?('Upgrade auf '+(LS_PLANS[extra.requested_plan]?.label||extra.requested_plan)):('+'+extra.pack_amount+' Credits für €'+extra.pack_price);
  try{
    const{error}=await getSB().from('admin_notifications').insert({
      ...
      message:`${master.salon_name||master.full_name||master.email||''}: ${label} (Plan: ${master.plan||currentUser.plan||'?'}, Rest: ${getRemaining()})`,
      payload:{...extra,current_plan:master.plan||currentUser.plan||null,remaining:getRemaining(),salon_name:master.salon_name||null,email:master.email||currentUser.email||null}
    });
    if(error)throw error;
    document.getElementById('coBox').innerHTML=`<div class="co-ok"><div class="co-ok-icon">✅</div>
      <div class="co-head">Anfrage gesendet!</div>
      <div class="co-sub">${label}<br>Wir melden uns umgehend bei Ihnen zur Bezahlung & Freischaltung.</div>
      <button class="co-close" onclick="closeCreditOffer()">Schließen</button></div>`;
  }catch(e){
...
function render(){
  ...
  const visibleModels=d.models.filter(m=>!_deletedModels.has(`${currentMode}|${String(m.id)}`));
...
function rebuildCurrentPrompt(){
 ...
 if(_basisMode&&currentUser?.is_admin&&isAdminApp()){
  if(_farbeMode){
   ...
   currentPrompt=buildBasisPrompt(`Service / color to display:\n${clean}`,hairType,_selectedAngle,modifierText);
  }else{
   const spec=englishSpecs[currentMode]?.[currentStyleId]||englishSpecs[currentStyleId]||currentStyleName;
   const hairType=currentMode==='beard'?'beard and facial hair':'hair on the head';
   let block=String(spec);
   if(_selectedColor)block+=`\n\nCOLOR TO APPLY:\n${_selectedColor.replace('COLOR:','').trim()}`;
   currentPrompt=buildBasisPrompt(block,hairType,_selectedAngle,modifierText);
  }
  return;
 }
 if(_farbeMode){
...
function ensureBasisToggle(){
 ...
 wrap.id='basisModeWrap';
 wrap.style.cssText='margin:10px 0;padding:10px 14px;border:1px dashed rgba(251,191,36,0.45);border-radius:12px;background:rgba(251,191,36,0.05)';
 wrap.innerHTML=`<label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:13px;line-height:1.45">
   <input type="checkbox" id="basisModeChk" style="margin-top:2px;accent-color:#fbbf24" onchange="setBasisMode2(this.checked)">
   <span><b style="color:#fbbf24">🧩 BASIS-MODUS (Admin)</b><br>
   <span style="opacity:.75">Erzeugt statt einer Kunden-Simulation ein neutrales <b>Basis-Bild</b>: ein Mannequin-Kopf ohne Gesicht, mit einer Brille — als Stil-Referenz für die KI. Danach „In Galerie" speichern und im Gallery-Admin mit ★ als Basis markieren.</span></span>
 </label>`;
 upload.parentNode.insertBefore(wrap,upload);
 return wrap;
}
function setBasisMode2(on){
 _basisMode=!!on;
 rebuildCurrentPrompt();
 const genBtn=document.getElementById('genBtn');
 if(genBtn)genBtn.textContent=_basisMode?'▸ Basis-Bild generieren':'▸ Simulation starten';
}
...
function resetGenState(){
  ...
  _basisMode=false;
  const _bmChk=document.getElementById('basisModeChk');if(_bmChk)_bmChk.checked=false;
    ...
    const { data:functionResponse, error:functionError } = await getSB().functions.invoke(IMAGE_FUNCTION_NAME, {
      body: {
        ...
        meta: {
          ...
          styleId: _basisMode ? null : (currentStyleId || null),
