...
function galColorMode(mode,gender){const cm=galCanonMode(mode);return (cm==='color'&&(gender||currentGender)==='male')?'color_herren':cm;}
...
async function deleteModelFromSoftware(mode,id,name){
  ...
  try{
    const modeSet=canon==='color'?['color','color_herren']:(canon==='behandlung'?['behandlung','treatment']:[canon]);
...
async function _batchSaveGallery(mode,modelId,modelName,beforeB64,afterB64){
  ...
  const segMap={female:'female/hair_cut',male:'male/hair_cut',beard:'male/beard_color',color:'color/service',color_herren:'color/service_herren',treatment:'behandlung/service'};
