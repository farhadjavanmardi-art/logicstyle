...
/* Farbe nach Geschlecht trennen: Herren-Farbe eigener Speicher-Modus, damit Damen/Herren nie mischen */
function galColorMode(mode,gender){const cm=galCanonMode(mode);const g=gender||currentGender;if(cm==='color'&&g==='male')return 'color_herren';if(cm==='behandlung'&&g==='male')return 'behandlung_herren';return cm;}
...
/* Modell endgültig aus der Software löschen — nur Admin (LS_ADMIN_REFS).
   Entfernt alle Galerie-Bilder + Storage-Dateien des Modells und blendet es
   über deleted_models dauerhaft aus Kunden- und Admin-Auswahl aus. */
function buildDeleteBtn(mode,id,name){
  if(!window.LS_ADMIN_REFS)return'';
  const n=String(name||'').replace(/\\/g,'\\\\').replace(/'/g,"\\'");
  return`<button class="btn-ref btn-del" style="color:#f87171;border-color:rgba(248,113,113,.3)" title="Modell endgültig löschen" onclick="event.stopPropagation();deleteModelFromSoftware('${mode}','${String(id).replace(/'/g,"")}','${n}')">🗑</button>`;
}
function storagePathFromUrl(u){const s=String(u||'');const i=s.indexOf('/simulations/');return i<0?'':s.substring(i+13).split('?')[0];}
async function deleteModelFromSoftware(mode,id,name){
  const canon=mode==='treatment'?'behandlung':mode;
  if(!confirm(`Modell "${name}" (#${id}) endgültig aus der Software löschen?\n\nAlle Galerie-Bilder dieses Modells werden entfernt und es verschwindet aus der Auswahl (Kunde & Admin).\n\nDiese Aktion kann nicht rückgängig gemacht werden.`))return;
  try{
    const modeSet=canon==='color'?['color','color_herren']:(canon==='behandlung'?['behandlung','treatment','behandlung_herren']:[canon]);
    const{data:rows}=await getSB().from('model_gallery').select('id,before_url,after_url,image_url').eq('model_id',String(id)).in('mode',modeSet);
    if(rows&&rows.length){
      const ids=rows.map(r=>r.id).filter(Boolean);
      const paths=[];
      rows.forEach(r=>[r.before_url,r.after_url,r.image_url].forEach(u=>{const p=storagePathFromUrl(u);if(p&&paths.indexOf(p)<0)paths.push(p);}));
      if(ids.length)await getSB().from('model_gallery').delete().in('id',ids);
      if(paths.length)try{await getSB().storage.from('simulations').remove(paths);}catch(_e){}
    }
    await getSB().from('deleted_models').upsert({mode:canon,id:String(id),name:String(name||'')});
    _deletedModels.add(`${canon}|${String(id)}`);
    try{await loadGalleryCounts();}catch(_e){}
    renderCurrent();
    alert(`✅ "${name}" wurde gelöscht`);
  }catch(e){alert('❌ Fehler beim Löschen: '+(e&&e.message||e));}
}

/* thumbnail کوچک عکس نمونه جلوی هر مدل (آخرین شبیه‌سازی) */
function buildThumb(id){
  const t=_galleryThumbs[`${galColorMode(currentMode)}|${id}`];
  if(!t||!t.after)return'';
...
function render(){
  ...
  cats.forEach(cat=>{
    ...
    items.forEach(m=>{
      ...
      row.innerHTML=`${buildThumb(m.id)||'<div class="model-dot"></div>'}<div class="model-info"><div class="model-name"><span class="model-num">#${m.id}</span>${m.name}</div></div><div class="model-row-right">${buildRefLinks(m.name)}${buildEditIcon(currentMode,m.id)}${buildGalleryBtn(currentMode,m.id,m.name)}${buildDeleteBtn(currentMode,m.id,m.name)}<button class="btn-simulate" onclick="openSim('${m.id}',event)">Simulieren ▸</button></div>`;
...
function renderServiceMode(sections, mode){
  ...
  sections.forEach(sec=>{
    ...
    items.forEach(item=>{
      ...
      const row=document.createElement('div');row.className='model-row';
      row.innerHTML=`${buildThumb(id)||(item.icon?`<div class="model-icon-box">${item.icon}</div>`:'<div class="model-dot"></div>')}<div class="model-info"><div class="model-name"><span class="model-num">#${item.id||''}</span>${item.name}</div></div><div class="model-row-right">${buildRefLinks(item.name)}${buildEditIcon(mode,item.id||id)}${buildGalleryBtn(mode,item.id||id,item.name)}${buildDeleteBtn(mode,item.id||id,item.name)}<button class="btn-simulate" onclick="openSim('${item.id||id}',event)">Simulieren ▸</button></div>`;
      list.appendChild(row);
    });
    block.appendChild(list);grid.appendChild(block);
  });
...
function getDbModeForSave(){
  ...
  if(currentMode==='treatment') return galColorMode('treatment');
  if(currentMode==='color')     return galColorMode('color');
...
async function _batchSaveGallery(mode,modelId,modelName,beforeB64,afterB64){
  const storeMode=galColorMode(mode);  // Herren-Farbe → 'color_herren' (getrennt von Damen-Farbe)
  const segMap={female:'female/hair_cut',male:'male/hair_cut',beard:'male/beard_color',color:'color/service',color_herren:'color/service_herren',treatment:'behandlung/service',behandlung:'behandlung/service',behandlung_herren:'behandlung/service_herren'};
  const seg=segMap[storeMode]||'male/hair_cut';const ts=Date.now();
  ...
  const{error}=await getSB().from('model_gallery').insert({
    model_id:String(modelId),mode:storeMode,image_url:afterUrl,before_url:beforeUrl,after_url:afterUrl,
...
async function startBatch(){
  ...
  for(const m of list){
    ...
    if(skipExisting&&(_galleryCountsByMode[`${galColorMode(mode)}|${m.id}`]||0)>0){skip++;done++;_batchSetBar(done/total*100);_batchLog(`↷ ${name} (hat bereits ein Bild)`);continue;}
...
async function openGallery(modelId,modelName,event){
  ...
  try{
    ...
    const _cm=galColorMode(currentMode);
