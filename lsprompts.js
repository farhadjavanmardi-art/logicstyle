...
function buildHairPrompt(spec, hairType, view, modifierText='') {
 const viewNote = `Detect the exact camera angle and head pose directly from the uploaded photo — do not force it into one of a few fixed positions. Support the full continuous 360-degree range: front, back, left profile, right profile, or any diagonal / three-quarter angle in between. Whatever angle, head rotation, tilt, and framing are visible in the original photo, replicate that EXACT same angle in the output. Do not rotate the client to a different pose, do not straighten a diagonal angle into a "clean" front/side/back view, do not invent a frontal face if the original is turned away or in profile, and keep the face fully invisible if the original only shows the back of the head. The output camera position must match the input photo's camera position precisely. (Reference label from the client's angle selection, for context only: ${view})`;
...
function buildColorPrompt(service, hairType, view, modifierText='') {
 ...
 const viewNote = `Detect the exact camera angle and head pose directly from the uploaded photo — do not force it into one of a few fixed positions. Support the full continuous 360-degree range: front, back, left side, right side, or any diagonal / three-quarter angle in between. Whatever angle, head rotation, tilt, and framing are visible in the original photo, replicate that EXACT same angle in the output and show the color/treatment/technique result exactly as it would be visible from that angle. Do not rotate the client to a different pose, do not straighten a diagonal angle into a "clean" front/side/back view, do not invent a frontal face if the original is turned away or in profile, and keep the face fully invisible if the original only shows the back of the head. The output camera position must match the input photo's camera position precisely. (Reference label from the client's angle selection, for context only: ${view})`;
 const modifierBlock = normalizeModifierText(modifierText) ? `\nADVANCED CLIENT / BARBER / SALON SETTINGS:\n${normalizeModifierText(modifierText)}\n` : '';
