...
/* ══ BASIS-MODUS — neutrales Referenz-Template (Mannequin-Kopf mit Brille) ══ */
function buildBasisPrompt(styleBlock, hairType, view, modifierText='') {
 const basisViews = {
  'FRONT VIEW':          'directly from the front (0°), head centered, both shoulders visible',
  'LEFT SIDE PROFILE':   'exact 90° left profile, showing the full left silhouette, sideburn line and neckline',
  'RIGHT SIDE PROFILE':  'exact 90° right profile, showing the full right silhouette, sideburn line and neckline',
  'BACK VIEW':           'directly from behind (180°), showing crown, back panel, neckline and overall silhouette',
  '45 DEGREE LEFT VIEW': '45° from the front-left, showing depth, temple, sideburn and outline',
  '45 DEGREE RIGHT VIEW':'45° from the front-right, showing depth, temple, sideburn and outline',
  'FRONT 45 RIGHT VIEW': '45° from the front-right, showing depth, temple and outline',
  'FRONT 45 LEFT VIEW':  '45° from the front-left, showing depth, temple and outline',
  'BACK 45 RIGHT VIEW':  '45° from the back-right, showing crown flow, occipital shape and neckline',
  'BACK 45 LEFT VIEW':   '45° from the back-left, showing crown flow, occipital shape and neckline',
 };
 const viewNote = basisViews[view] || `from this perspective: ${view}`;
 const isBeard = hairType === 'beard and facial hair';
 const subjectArea = isBeard ? 'beard, mustache and facial hair' : 'hair on the head';
 const modifierBlock = normalizeModifierText(modifierText)
  ? `\nADVANCED STYLE SETTINGS:\n${normalizeModifierText(modifierText)}\n` : '';

 return `You are creating a NEUTRAL STYLE REFERENCE / BASE TEMPLATE image for the Logic Style salon software — NOT a client consultation preview.

PURPOSE:
This image will be stored as the official BASE IMAGE of exactly ONE style. It will later be shown to the AI as a style reference NEXT TO real client photos. It must therefore be instantly recognizable as an artificial template and NEVER look like a real person.

TEMPLATE SUBJECT — NON-NEGOTIABLE:
- Exactly ONE subject: a single neutral, faceless, mannequin-like head-and-shoulders bust — never two people, never a collage, never a grid, never multiple angles combined into one image
- NO recognizable face and NO real facial identity: a smooth, neutral, matte skin-toned surface with no detailed eyes, no eyebrows and no expression — at most minimal soft indications of nose and lips
- The mannequin wears exactly ONE pair of simple black rectangular glasses. The glasses mark the image as a template. No other accessories: no earrings, no jewelry, no visible clothing details
- Completely IGNORE and REPLACE the person, face, identity, background and clothing of the uploaded photo — use it only as loose framing guidance. The output must NOT resemble the uploaded person in any way
- Plain, even, light-gray-to-white studio background; soft neutral studio lighting; clean head-and-shoulders framing
- No text, no watermark, no logo, no captions, no graphic overlays

STYLE TO DISPLAY — THIS IS THE ONLY CONTENT THAT MATTERS:
${styleBlock}
${modifierBlock}
STYLE EXECUTION RULES:
- Render the specified ${subjectArea} on the mannequin head with maximum technical accuracy and photorealism: real hair texture, density, growth direction and physics
- Cut structure, lengths, proportions, fade placement, outline, line definition, density, color placement and overall silhouette must match the specification EXACTLY — this image DEFINES the style
- Show every distinctive detail of the specification clearly, so this style cannot be confused with any similar style
- ${isBeard ? 'Show ONLY the specified beard/mustache design; the head hair stays a short, neutral, unstyled default so it never distracts from the beard style' : 'Show ONLY the specified hairstyle; if no beard is part of the specification, keep the mannequin clean-shaven'}
- Hair color: use a neutral medium-brown tone unless the specification explicitly defines a color

VIEW / ANGLE:
Show the mannequin ${viewNote}. One single view only.

OUTPUT:
One photorealistic reference image: a single faceless mannequin-like head wearing glasses, displaying exactly the specified style, on a plain neutral background.`;
}
